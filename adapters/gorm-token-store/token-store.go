...
func (s *GormTokenStore) Create(ctx context.Context, info oauth2.TokenInfo) error {
	...
	if err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method Create when tokenToRow: %s", err.Error()))

		return err
	}

	if err = s.db.WithContext(ctx).Create(row).Error; err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method Create when create DB: %s", err.Error()))
...
func (s *GormTokenStore) RemoveByCode(ctx context.Context, code string) error {
	if err := s.db.WithContext(ctx).Where("code = ?", code).Delete(&models.OAuthToken{}).Error; err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method RemoveByCode: %s", err.Error()))
...
func (s *GormTokenStore) RemoveByAccess(ctx context.Context, access string) error {
	if err := s.db.WithContext(ctx).Where("access = ?", access).Delete(&models.OAuthToken{}).Error; err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method RemoveByAccess: %s", err.Error()))
...
func (s *GormTokenStore) RemoveByRefresh(ctx context.Context, refresh string) error {
	if err := s.db.WithContext(ctx).
		...
		logger.Log.Error(fmt.Printf("failed GormTokenStore method RemoveByRefresh: %s", err.Error()))
...
func (s *GormTokenStore) GetByCode(ctx context.Context, code string) (oauth2.TokenInfo, error) {
	...
	if err := s.db.WithContext(ctx).
		...
		logger.Log.Error(fmt.Printf("failed GormTokenStore method GetByCode: %s", err.Error()))
	...
	if err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method GetByCode when rowToToken: %s", err.Error()))
...
func (s *GormTokenStore) GetByAccess(ctx context.Context, access string) (oauth2.TokenInfo, error) {
	...
	if err := s.db.WithContext(ctx).
		...
		First(&row).Error; err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method GetByAccess: %s", err.Error()))
		if errors.Is(err, gorm.ErrRecordNotFound) {
	...
	tokenInfo, err := rowToToken(&row)
	if err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method GetByAccess when rowToToken: %s", err.Error()))
...
func (s *GormTokenStore) GetByRefresh(ctx context.Context, refresh string) (oauth2.TokenInfo, error) {
	...
	if err := s.db.WithContext(ctx).
		...
		First(&row).Error; err != nil {
		logger.Log.Error(fmt.Printf("failed GormTokenStore method GetByRefresh: %s", err.Error()))
		if errors.Is(err, gorm.ErrRecordNotFound) {
	...
	if err != nil {
		logger.Log.Error(
			fmt.Printf("failed GormTokenStore method GetByRefresh when rowToToken: %s", err.Error()),
		)
