...
import (
	...
	"booking/internal/app/core/http"
	"booking/internal/app/core/rules"
	"booking/pkg/cache"
	"booking/pkg/gormtx"
	"booking/pkg/postgres"
	"booking/pkg/validation"
	"context"
	"fmt"
	"os"
	"os/signal"
...
func (s *Server) initLayers(_ context.Context) error {
	...
	s.txm = gormtx.NewManager(s.pgdb)
	...
	s.cache = cache.NewRedisCache(s.rdb, "booking")
...
func (s *Server) initServer(_ context.Context) error {
	...
	select {
	case serv := <-interrupt:
		fmt.Println("kinoqor-booking-api - запущен - signal: " + serv.String())
	case err := <-httpServer.Notify():
		fmt.Println(fmt.Errorf("kinoqor-booking-api - запущен - httpServer.Notify: %w", err))
	}

	if err := httpServer.Shutdown(); err != nil {
		fmt.Println(fmt.Errorf("kinoqor-booking-api - запущен - httpServer.Shutdown: %w", err))
	}
