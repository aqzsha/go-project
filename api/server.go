...
func (s *Server) initServer(_ context.Context) error {
	...
	select {
	case serv := <-interrupt:
		fmt.Println("kinokor-auth-api - запущен - signal: " + serv.String())
	case err := <-httpServer.Notify():
		fmt.Println(fmt.Errorf("kinokor-auth-api - запущен - httpServer.Notify: %w", err))
	}
	
	if err := httpServer.Shutdown(); err != nil {
		fmt.Println(fmt.Errorf("kinokor-auth-api - запущен - httpServer.Shutdown: %w", err))
	}
