...
func (s *Server) initServer(_ context.Context) error {
	...
	case serv := <-interrupt:
		...
		logger.Log.Println("do-auth-api - запущен - signal: " + serv.String())
	case err := <-httpServer.Notify():
		fmt.Println(fmt.Errorf("do-auth-api - запущен - httpServer.Notify: %w", err))
		logger.Log.Println(fmt.Errorf("do-auth-api - запущен - httpServer.Notify: %w", err))
	}

	if err := httpServer.Shutdown(); err != nil {
		fmt.Println(fmt.Errorf("do-auth-api - запущен - httpServer.Shutdown: %w", err))
		logger.Log.Println(fmt.Errorf("do-auth-api - запущен - httpServer.Shutdown: %w", err))
	}
